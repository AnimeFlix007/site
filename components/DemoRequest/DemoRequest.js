...
const DemoRequest = () => {
    ...
    const handleSubmit = async(e) => {
        ...
        try {
            ...
            const { name, email, number, subject, role, details, comments } = contact;
            const payload = { name, email, number, subject, text: JSON.stringify({ role, details, comments }) };
            const response = await axios.post(url, payload);
        
            console.log({ name, email, subject, text: JSON.stringify({ number, role, details, comments }) })
            alertContent();
